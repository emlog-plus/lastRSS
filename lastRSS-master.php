...
require_once(EMLOG_ROOT.'/content/plugins/lastRSS-master/lastRSS_class.php');
...
function lastRSS_css(){
	echo '<link rel="stylesheet" href="'.BLOG_URL.'content/plugins/lastRSS-master/css/lastRSS.css" />'."\n";
}
function lastRSS_ajax(){
	echo '<script type="text/javascript" src="'.BLOG_URL.'content/plugins/lastRSS-master/js/jquery-autoscroll.js"></script>'."\n";
	echo '<script type="text/javascript">$(function(){$.get("'.BLOG_URL.'index.php?plugin=lastRSS-master&action=update&r="+Math.random());$("#lastRSS").load("'.BLOG_URL.'index.php?plugin=lastRSS-master&action=ajaxshow&r="+Math.random(),function(){$(this).removeClass("lastRSS_loding").autoScroll()})})</script>'."\n";
}
function lastRSS_adminUpdate(){
	echo '<script type="text/javascript">$(function(){$.get("'.BLOG_URL.'index.php?plugin=lastRSS-master&action=update&r="+Math.random());})</script>'."\n";
}
function lastRSS_menu(){
	echo '<li><a href="./plugin.php?plugin=lastRSS-master" id="lastRSS">Rss订阅</a></li>';
}
